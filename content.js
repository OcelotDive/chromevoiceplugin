...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "check if video command") {
    let trimRequest = request.transcript
      .split(" ")
      .map((e) => e.trim())
      .filter((e) => e !== "")
      .join(" ")
    ...
    if (trimRequest.includes("save video")) {
      ...
      const currentTimePlayed = document.querySelector(".ytp-time-current")
        .innerHTML;
      let currentVideoPlayed = document.querySelector("h1.title").textContent;
      if (currentVideoPlayed.length > 25)
        currentVideoPlayed = currentVideoPlayed.substring(0, 25) + "..";
    ...
    iframes.forEach((iframe) => {
      ...
      if (iframe.src.includes("youtube.com")) {
        let newSrc = iframe.src
          .substring(0, iframe.src.indexOf("?"))
...
function addElementLabelsToPage(displayModeBool) {
  ...
  let elementsList = Array.from(
    document.querySelectorAll("a, input, .tab-content")
  );
